...
## Approaching Convolution with Templates

In general, making template code to colvolve two templated types in a type-safe language like C++ will be a two-part problem: that of determining the type of the result, and the value.

### Determining the Type

Type determination must be done statically, and is usually the less intuitive of the two parts. It helps greatly to start by breaking down the problem into smaller, more manageable "detail" templates.

A simple place to start is a rank-determining template. The rank of a tensor is the number of array dimensions needed to represent it. For example `tensor<float, 2, 5, 1> t` is rank-3 because it could also be represented by `float t[2][5][1]`.

The problem of determining the rank of the tensor type amounts to counting the number of integral template parameters. This can be done with a recursive template definition which strips off one of the parameters and increments a static constant at each iteration, terminating in the case where there is a single template parameter left.
...
`enum` was used here instead of `static const int` in order to store `value` as a non-lvalue.
