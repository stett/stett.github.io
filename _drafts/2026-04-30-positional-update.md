...
The last section of Part 1, I introduced the Buamgarte term to correct for constraint drift. However, this factor requires tuning. It can result in overly relaxed constraints if it's too low, and can make the system unstable if it's too high. In this part, I'll introduce a positional correction iteration to the solver to attempt a reduction in required iteration count and an increase in stability.

<!-- excerpt -->

TODO: Justification

My thought here is to resolve constraint drift in its own step, rather than to let it sneak into the dynamics via additional potentials. 
